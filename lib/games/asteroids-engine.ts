...
export function createAsteroidsEngine(
  ...
  // ── Estado del juego (por instancia, sin globales de módulo) ────────────
  let ship = new Ship();
  let bullets: Bullet[] = [];
  ...
  function update(dt: number) {
    if (state === "gameover") {
      // El disparo de reinicio por Espacio se elimina a propósito:
      // solo "JUGAR DE NUEVO" (React) reinicia la partida.
